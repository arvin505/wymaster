...
public class YoukuPlayerView extends PluginOverlay implements DetailMessage,
	...
	private void initLayout() {
		...
		surfaceBlack = (View) findViewById(R.id.surface_black); //播放页面的黑色背景
	...
	//	Log.i(TAG, "onBufferingUpdateListener：：："+percent);
...
//				LinearLayout.LayoutParams.WRAP_CONTENT));
	...
	public void resizeVideoView(int percent, boolean force) {
		...
		// 控件的宽高
		fullHeight = playback.getHeight();  //608
		fullWidth = playback.getWidth();  //1080
		int orientation = mMediaPlayerDelegate.mediaPlayer == null ? 0
				: mMediaPlayerDelegate.mediaPlayer.getVideoOrientation();  //0
		...
		if (percent == -1) {// 满屏
			...
			int videoHeight = 0;
			int videoWidth = 0;
			if (orientation == 0 || orientation == 3) {  //竖屏
           ...
           //刚进入页面的时候 videoWidth为0   播放后初始化mVideoWidth
			if (videoWidth == 0) 
				return;
...
//	}
	...
	public void onVideoInfoGetting() {
		...
		setPlayerBlack();  //加载视频信息时候设置播放界面为黑色
		realVideoStart = false;
	}
