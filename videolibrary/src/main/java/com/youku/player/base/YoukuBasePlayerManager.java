...
public abstract class YoukuBasePlayerManager extends IBasePlayerManager
	...
	private static final String TAG1="YoukuBasePlayerManager";
	public YoukuBasePlayerManager(Activity context) {
		super(context);
		this.controlTitleShowHide=(PluginSimplePlayer.ControlTitleShowHide)context;
	}
	...
	private boolean isSystemLockScreenBack=false;
	...
	protected void addPlugins() {
		getBaseActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				((PluginSimplePlayer)mPluginSmallScreenPlay).setContext(controlTitleShowHide);
...
//	private DeviceOrientationHelper orientationHelper;
	...
	//	unregisterHomeKeyReceiver();
		...
		if (mYoukuPlayerView!=null && !mYoukuPlayerView.firstOnloaded) {
	...
	public void onStop() {
		super.onStop();
		if(mPluginSmallScreenPlay!=null && (mPluginSmallScreenPlay instanceof PluginSimplePlayer)){
			((PluginSimplePlayer)mPluginSmallScreenPlay).hideAll();
			((PluginSimplePlayer)mPluginSmallScreenPlay).removeHideHandler();
		}
	}
...
//			}
			//销毁的时候移除插件  否则插件会一层叠加一层
			if(pluginManager!=null){
				pluginManager.removePlugin(mPluginFullScreenPlay,player_holder);
				pluginManager.removePlugin(mPluginSmallScreenPlay,player_holder);
			}
	...
	public boolean getSystemLockScreenState(){
		return  isSystemLockScreenBack;
	}
	public void setSystemLockScreenState(boolean state){
		isSystemLockScreenBack=state;
	}
...
//	}
	...
	public void setClickChange(boolean change){
		isClickChange=change;
	}
	public boolean getClickChangeState(){
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		//boolean value=isChange==true?false:isLand();
		// 横屏有两种情�?�?
		if (isLand() && isClickChange) {
...
//			orientationHelper.fromUser = false;
	...
	protected void changeConfiguration(Configuration newConfig) {
		...
		if (isLand()) {
			...
			// 不是平板去全�?
			if (!PlayerUtil.isYoukuTablet(getBaseActivity())) {
	//			Log.i("onVideoSizeChanged", "!PlayerUtil.isYoukuTablet(getBaseActivity())");
				onFullscreenListener();
				getBaseActivity().closeOptionsMenu();
				setPlayerFullScreen(true);
				getBaseActivity().runOnUiThread(new Runnable() {
			...
			// if(Profile.PLANTFORM == Plantform.TUDOU && onPause) return;
//			if(mPluginSmallScreenPlay!=null && mPluginSmallScreenPlay instanceof  PluginSimplePlayer){
//				//home 或锁屏 返回到小平后要关闭 标题 锁屏键
//				((PluginSimplePlayer)mPluginSmallScreenPlay).hideControl2();
//			}
			goSmall();
			updatePlugin(!mediaPlayerDelegate.isAdvShowFinished() ? PLUGIN_SHOW_AD_PLAY
					: PLUGIN_SHOW_NOT_SET);
...
//		if(mPluginSmallScreenPlay!=null && (mPluginSmallScreenPlay instanceof PluginSimplePlayer)){
//			((PluginSimplePlayer)mPluginSmallScreenPlay).onContainerClick();
//		}
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		try {
			...
			case KeyEvent.KEYCODE_BACK:
				...
				//如果是锁屏状态  静止切换到小屏
				if(mPluginSmallScreenPlay!=null && (mPluginSmallScreenPlay instanceof PluginSimplePlayer)
						&& ((PluginSimplePlayer)mPluginSmallScreenPlay).getLockScrennState()){
					return true;
				}
				if (!mediaPlayerDelegate.isDLNA) {
					if (mediaPlayerDelegate.isFullScreen
						...
						if(mPluginSmallScreenPlay!=null && (mPluginSmallScreenPlay instanceof PluginSimplePlayer)){
							((PluginSimplePlayer)mPluginSmallScreenPlay).hideControl2();
							((PluginSimplePlayer)mPluginSmallScreenPlay).controlTitleShowHide.showTitle();
						}
						goSmall();   //返回键返回
						return true;
					} else {
						if(mYoukuPlayerView!=null && mYoukuPlayerView.getVisibility()==View.VISIBLE){
							mYoukuPlayerView.setVisibility(View.GONE);
							onPause();
							onStop();
							onDestroy();
						}else {
...
//		Log.i(TAG1, "onFullScreenPlayComplete::");
////		if (null != orientationHelper)
////			orientationHelper.disableListener();
////		getBaseActivity().setRequestedOrientation(
////				ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
...
//			orientationHelper.enableListener();
	...
	public static void setSuggestVideoType(String VideoType, Context context) {
		Log.i(TAG1, "setSuggestVideoType::"+VideoType);
	...
	private void showPasswordInputDialog(int title) {
		LayoutInflater factory = LayoutInflater
				.from(YoukuPlayerConfiguration.context);
...
//		getBaseActivity().registerReceiver(mHomeKeyReceiver, homeFilter);
//	}

//	private  void unregisterHomeKeyReceiver() {
//		if (null != mHomeKeyReceiver) {
//			getBaseActivity().unregisterReceiver(mHomeKeyReceiver);
//		}
//	}
...
//
//	}
	PluginSimplePlayer.ControlTitleShowHide controlTitleShowHide;
