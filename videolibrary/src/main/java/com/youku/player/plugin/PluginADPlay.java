...
public class PluginADPlay extends PluginOverlay implements DetailMessage {
	...
	private void init(Context context) {
		...
		if (Profile.PLANTFORM == Plantform.YOUKU) {
			 ...
			 * Intent(); intent.setClassName(mActivity.getPackageName(),
			 * "com.youku.phone.vip.activity.VipProductActivity");
			 * intent.putExtra("from", 1001); intent.putExtra("isVip", false);
	...
	public void notifyUpdate(int count) {
		...
		int visibility = mediaPlayerDelegate.isPlayLocalType() ? View.GONE
				: View.VISIBLE;
		...
		if (mediaPlayerDelegate.videoInfo.videoAdvInfo != null) {
			...
			if (advInfo == null) {
				Logger.e("PlayFlow",
						"PlugiADPlay->notifyUpdate    advInfo = null,   return");
				return;
			}

			/*
			 * if (TextUtils.isEmpty(advInfo.CU)) {
			 * ad_more.setVisibility(View.GONE); } else { if
			 * (AdForward.YOUKU_VIDEO == advInfo.CUF) {
			 * ad_more.setText(R.string.playersdk_ad_descrip_play_youku); } else
	...
	public void onPluginAdded() {
		...
		if (mediaPlayerDelegate.videoInfo.videoAdvInfo != null) {
			 ...
			 * 
			 * if (adInfo.VAL.size() <= 0) { return; }
			 * 
			 * AdvInfo advInfo = adInfo.VAL.get(0); if (advInfo == null) {
			 * return; } String url = advInfo.CU; Logger.e("PlayFlow",
			 * "点击url-->" + url);
			 * 
			 * if (url == null || TextUtils.getTrimmedLength(url) <= 0) {
			 * return; } DisposableStatsUtils.disposeCUM(advInfo); new
			 * AdvClickProcessor().processAdvClick(mActivity, url, advInfo.CUF);
			 * } });
			 */
		}
	...
	public void setSkipVisible(boolean visible) {
		/*
		 * if (MediaPlayerConfiguration.getInstance().showSkipAdButton() &&
		 * mAdSkip != null) { mAdSkip.setVisibility(visible ? View.VISIBLE :
		 * View.GONE); if (mAdSkipBlank != null) {
