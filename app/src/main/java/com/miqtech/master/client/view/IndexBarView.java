...
public class IndexBarView extends View {
    ...
    public void setData(PinnedHeaderListView listView, List<Object> listItems,List<Integer> listSections) {
        ...
        mIndexPaint.setTextSize(mContext.getResources().getDimension(R.dimen.text_size_12));
    }
    ...
    public String getSectionText(int sectionPosition) {
    	if(mListItems.get(sectionPosition) instanceof City){
    		return ((City)mListItems.get(sectionPosition)).getName();
    	}else{
    		return (String)mListItems.get(sectionPosition);
    	}
    }
    ...
    void filterListItem(float sideIndexY) {
        ...
        if (mCurrentSectionPosition >= 0 && mCurrentSectionPosition < mListSections.size()) {
            ...
            if(mListItems.get(position) instanceof City){
            	previewText = ((City)mListItems.get(position)).getName();
            }else{
            	previewText = (String)mListItems.get(position);
            }
