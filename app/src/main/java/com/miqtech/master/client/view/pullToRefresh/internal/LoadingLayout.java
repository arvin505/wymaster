...
abstract class LoadingLayout extends LoadingLayoutBase {
	...
	public LoadingLayout(Context context, final PullToRefreshBase.Mode mode, final PullToRefreshBase.Orientation scrollDirection, TypedArray attrs) {
		...
		switch (scrollDirection) {
			case HORIZONTAL:
				LayoutInflater.from(context).inflate(R.layout.pr_pull_to_refresh_header_horizontal, this);
				break;
			case VERTICAL:
			default:
				LayoutInflater.from(context).inflate(R.layout.pr_pull_to_refresh_header_vertical, this);
				break;
		}
     ...
     //TODO 处理label
		switch (mode) {
			case PULL_FROM_END:
				...
				mReleaseLabel = context.getString(R.string.pr_pull_to_refresh_from_bottom_refreshing_label);
				break;
